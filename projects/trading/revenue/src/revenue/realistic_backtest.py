...
class RealisticDataManager:
    ...
    def generate_realistic_market_data(
        self, period: MarketPeriod, hours: int = 1000
    ) -> list[dict]:
...
class MarketNeutralStrategies:
    ...
    def correlation_breakdown_strategy(
        self, btc_data: list[dict], eth_data: list[dict], index: int
    ) -> dict | None:
        ...
        eth_returns = [
            (eth_prices[i] - eth_prices[i - 1]) / eth_prices[i - 1]
        ...
        numerator = sum(
            (br - mean_btc) * (er - mean_eth)
            for br, er in zip(btc_returns, eth_returns, strict=False)
        )
        ...
        hist_btc_prices = [
            d["close"] for d in btc_data[max(0, index - 50) : index - 20]
        ]
        hist_eth_prices = [
            d["close"] for d in eth_data[max(0, index - 50) : index - 20]
        ]

        hist_btc_returns = [
            (hist_btc_prices[i] - hist_btc_prices[i - 1]) / hist_btc_prices[i - 1]
            for i in range(1, len(hist_btc_prices))
        ]
        hist_eth_returns = [
            (hist_eth_prices[i] - hist_eth_prices[i - 1]) / hist_eth_prices[i - 1]
        ...
        hist_numerator = sum(
            (br - hist_mean_btc) * (er - hist_mean_eth)
            for br, er in zip(hist_btc_returns, hist_eth_returns, strict=False)
        )
    ...
    def mean_reversion_strategy(self, data: list[dict], index: int) -> dict | None:
    ...
    def momentum_strategy(self, data: list[dict], index: int) -> dict | None:
        ...
        if (
            abs(momentum) > 0.02 and volume_ratio > 1.5
        ):  # 2%+ momentum with 50%+ higher volume
...
class RealisticBacktester:
    ...
    def execute_trade(
        self, opportunity: dict, current_price: float, timestamp: datetime, symbol: str
    ):
        ...
        )  # Reduce by 20% for realism

        if random.random() < success_probability:
            # Winning trade
            gross_profit = (
                position_size * opportunity["expected_return"] * 0.7
            )  # 70% of expected
    ...
    def run_period_backtest(self, period: MarketPeriod) -> dict:
        ...
        data_hours = min(
            1000, int((period.end_date - period.start_date).total_seconds() / 3600)
        )
        ...
        for i in range(100, len(btc_data)):  # Start after enough history
            ...
            corr_opp = self.strategies.correlation_breakdown_strategy(
                btc_data, eth_data, i
            )
            if corr_opp:
                self.execute_trade(
                    corr_opp, btc_data[i]["close"], current_timestamp, "BTCETH_PAIR"
                )
                trades_in_period += 1
            ...
            for symbol, data in [("BTC", btc_data), ("ETH", eth_data)]:
                ...
                if mean_rev_opp:
                    self.execute_trade(
                        mean_rev_opp, data[i]["close"], current_timestamp, symbol
                    )
                    trades_in_period += 1

                # Test momentum (directional)
                momentum_opp = self.strategies.momentum_strategy(data, i)
                if momentum_opp and period.market_type in [
                    "bull",
                    "bear",
                ]:  # Only in trending markets
                    self.execute_trade(
                        momentum_opp, data[i]["close"], current_timestamp, symbol
                    )
                    trades_in_period += 1
    ...
    def calculate_realistic_metrics(
        ...
        bull_returns = [
            r["strategy_return"] for r in period_results if "bull" in r["period_name"]
        ]
        bear_returns = [
            r["strategy_return"] for r in period_results if "bear" in r["period_name"]
        ]
        sideways_returns = [
            r["strategy_return"]
            for r in period_results
            if "sideways" in r["period_name"]
        ]
        ...
        sideways_market_return = (
            statistics.mean(sideways_returns) if sideways_returns else 0
        )
        ...
        profit_factor = (
            abs(sum(winning_pnls) / sum(losing_pnls)) if losing_pnls else float("inf")
        )
...
def print_realistic_results(metrics: RealisticBacktestMetrics, initial_capital: float):
    ...
    print(
        f"   Final Capital:          ${initial_capital * (1 + metrics.total_return):,.2f}"
    )
    ...
    if metrics.excess_return > 0:
        ...
        print(
            f"   ❌ Strategy underperforms buy & hold by {abs(metrics.excess_return) * 100:.2f}%"
        )

    if metrics.bear_market_return > -0.1:
        print("   ✅ Decent bear market protection")
    else:
        print("   ❌ Poor bear market performance")
...
def main():
    ...
    if metrics.excess_return > 0.05 and metrics.bear_market_return > -0.2:
        print("   🚀 Strategy shows promise for MomoAI funding!")
        ...
        print("   📈 Strategy beats buy & hold but needs improvement")
    else:
        print("   ❌ Strategy needs major rework - doesn't beat buy & hold")
