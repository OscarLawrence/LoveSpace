...
class ContinuousTrader:
    ...
    def __init__(self):
        ...
        USDC_balance = (
            balances.get("USDC") or balances.get("USDC") or balances.get("BUSD")
        )

        if not USDC_balance or USDC_balance.total < 1:
            raise ValueError(
                f"Insufficient balance: ${USDC_balance.total if USDC_balance else 0:.2f}"
            )
        ...
        self.logger.info(
            f"🚀 Continuous Trader initialized with ${self.start_balance:.2f}"
        )
    ...
    def execute_trade(
        ...
        self.logger.info(
            f"{result_emoji} {opportunity['strategy']}: {symbol} @ ${current_price:.2f}"
        )
        self.logger.info(
            f"   Position: ${position_size:.2f}, P&L: ${simulated_pnl:+.2f}"
        )
    ...
    def trading_cycle(self):
        ...
        for symbol, data, price in [
            ("BTCUSDC", btc_data, current_btc),
            ("ETHUSDC", eth_data, current_eth),
        ]:
