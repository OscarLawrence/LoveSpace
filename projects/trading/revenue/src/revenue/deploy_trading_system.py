...
class LiveTradingEngine:
    ...
    def __init__(self, initial_capital: float = None):
        ...
        if not stable_balances:
            raise ValueError(
                f"No stablecoin balance found. Balances: USDC={USDC_balance.total if USDC_balance else 0:.2f}, USDC={usdc_balance.total if usdc_balance else 0:.2f}"
            )
        ...
        if USDC_balance.total < 1:
            raise ValueError(
                f"Insufficient {balance_currency} balance: ${USDC_balance.total:.2f}"
            )

        # For demo with small balance
        if USDC_balance.total < 100:
            self.logger.warning(
                f"⚠️  Small {balance_currency} balance detected: ${USDC_balance.total:.2f}"
            )
    ...
    def calculate_correlation_breakdown(
        self, btc_data: list[dict], eth_data: list[dict]
    ) -> dict | None:
    ...
    def calculate_mean_reversion(self, data: list[dict]) -> dict | None:
    ...
    def calculate_momentum(self, data: list[dict]) -> dict | None:
    ...
    def calculate_position_size(self, opportunity: dict) -> float:
        ...
        position_fraction = min(
            kelly_fraction * safety_factor, self.max_position_percent
        )
    ...
    def execute_trade(
        self, opportunity: dict, symbol: str, current_price: float
    ) -> bool:
        """Execute a live trade."""
        position_size = self.calculate_position_size(opportunity)

        min_position = (
            10 if self.capital < 100 else 20
        )  # Lower minimum for small accounts
        ...
        try:
            ...
            self.logger.info("   ✅ Trade recorded (demo mode)")
            return True
    ...
    def scan_opportunities(self) -> int:
        ...
        self.logger.info(
            f"📊 Market Scan: BTC=${current_btc_price:,.0f}, ETH=${current_eth_price:,.0f}"
        )
        ...
        for symbol, data, price in [
            ("BTCUSDC", btc_data, current_btc_price),
    ...
    def print_status(self):
        ...
        recent_trades = [
            t for t in self.trade_history if (datetime.now() - t["timestamp"]).days < 7
        ]
    ...
    def run_live_trading(self, duration_hours: int = 24):
        ...
        try:
            while datetime.now() < end_time:
                ...
                # Wait for next cycle
                self.logger.info(
                    f"⏰ Waiting {self.scan_interval} seconds until next scan..."
                )
...
def main():
    ...
    try:
        ...
        print(
            f"🎯 Max Position Size: ${engine.capital * engine.max_position_percent:.2f} (2%)"
        )
