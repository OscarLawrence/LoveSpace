...
def generate_realistic_price_data(
    days: int = 30, initial_price: float = 50000.0
) -> list:
...
def generate_trading_signals(price_data: list) -> list:
    ...
    )  # Lower collaboration weight for more signals
...
def main():
    ...
    for i, sig in enumerate(signals[:5]):
        print(
            f"  {i + 1}. {sig['timestamp']}: {sig['signal']} (confidence: {sig['confidence']:.2f})"
        )

    risk_params = {"max_position_size": 0.1, "stop_loss": 0.02, "take_profit": 0.05}
