...
class KellyPositionSizer:
    ...
    def calculate_kelly_fraction(
        self, win_prob: float, expected_return: float, risk_level: float
    ) -> float:
        """
    ...
    def calculate_position_size(
        ...
        kelly_fraction = self.calculate_kelly_fraction(
            confidence, expected_return, risk_level
        )
...
class VolatilityAdjustedSizer:
    ...
    def calculate_volatility_adjusted_position(
        ...
        volatility_adjustment = min(
            1.0, self.base_volatility / max(current_volatility, 0.001)
        )
...
def create_position_sizer(
    safety_factor: float = 0.25, max_position: float = 0.02
) -> KellyPositionSizer:
...
def create_volatility_sizer(
    ...
    kelly_sizer = KellyPositionSizer(
        safety_factor=safety_factor, max_position=max_position
    )
    return VolatilityAdjustedSizer(
        kelly_sizer=kelly_sizer, base_volatility=base_volatility
    )
