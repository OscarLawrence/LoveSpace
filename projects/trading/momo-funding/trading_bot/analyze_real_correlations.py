...
def analyze_real_market_correlations():
    ...
    try:
        ...
        system = OptimalTradingSystem(
            min_confidence=0.60, max_position=0.015
        )  # Lower confidence for analysis
        ...
        for i in range(len(asset_list)):
            for j in range(i + 1, len(asset_list)):
                ...
                if len(data1) >= 100 and len(data2) >= 100:
                    ...
                    recent_returns1 = system.altcoin_detector._calculate_returns(
                        recent_prices1
                    )
                    recent_returns2 = system.altcoin_detector._calculate_returns(
                        recent_prices2
                    )
                    mid_returns1 = system.altcoin_detector._calculate_returns(
                        mid_prices1
                    )
                    mid_returns2 = system.altcoin_detector._calculate_returns(
                        mid_prices2
                    )
                    hist_returns1 = system.altcoin_detector._calculate_returns(
                        historical_prices1
                    )
                    hist_returns2 = system.altcoin_detector._calculate_returns(
                        historical_prices2
                    )

                    if (
                        len(recent_returns1) >= 10
                        and len(mid_returns1) >= 10
                        and len(hist_returns1) >= 10
                    ):
                        recent_corr = system.altcoin_detector._calculate_correlation(
                            recent_returns1, recent_returns2
                        )
                        mid_corr = system.altcoin_detector._calculate_correlation(
                            mid_returns1, mid_returns2
                        )
                        hist_corr = system.altcoin_detector._calculate_correlation(
                            hist_returns1, hist_returns2
                        )
                        ...
                        recent_volume1 = (
                            sum(d["volume"] for d in data1[-24:])
                            / 24
                            * data1[-1]["close"]
                        )
                        recent_volume2 = (
                            sum(d["volume"] for d in data2[-24:])
                            / 24
                            * data2[-1]["close"]
                        )
                        ...
                        correlation_stats.append(
                            {
                                ...
                                "asset1_rank": system.optimal_universe.get(
                                    asset1, type("obj", (object,), {"rank": 999})
                                ).rank,
                                "asset2_rank": system.optimal_universe.get(
                                    asset2, type("obj", (object,), {"rank": 999})
                                ).rank,
                            }
                        )
        ...
        for i, stat in enumerate(correlation_stats[:10], 1):
            ...
            print(
                f"      Market Cap Ranks: {stat['asset1_rank']}-{stat['asset2_rank']}"
            )
            ...
            if stat["max_breakdown"] > 0.3:
                ...
                print("      📊 Below threshold (need >0.3)")
            print()
        ...
        if correlation_stats:
            ...
            print(
                f"   Opportunities at >0.3 threshold: {opportunities_03} ({opportunities_03 / len(correlation_stats):.1%})"
            )
            print(
                f"   Opportunities at >0.4 threshold: {opportunities_04} ({opportunities_04 / len(correlation_stats):.1%})"
            )
            print(
                f"   Opportunities at >0.5 threshold: {opportunities_05} ({opportunities_05 / len(correlation_stats):.1%})"
            )

            print("\n💡 REAL MARKET INSIGHTS:")
            if max_breakdown > 0.4:
                print("   ✅ Significant breakdowns exist in real data")
                print("   🎯 Current thresholds appear reasonable")
            elif max_breakdown > 0.3:
                print("   ⚠️  Moderate breakdowns found - consider lowering threshold")
                print("   🔧 Suggest 0.25-0.3 threshold for more opportunities")
            else:
                print("   ❌ Limited breakdowns in current timeframe")
                print("   🔄 May need longer observation period or different assets")

            # Volume analysis
            volumes = [
                s["avg_volume"] for s in correlation_stats if s["avg_volume"] > 0
            ]
            if volumes:
                avg_volume = sum(volumes) / len(volumes)
                print(f"   💰 Average trading volume: ${avg_volume:,.0f}/day")
                if avg_volume > 50000:
                    print("   ✅ Sufficient liquidity for execution")
                else:
                    print("   ⚠️  May need larger positions or different assets")
