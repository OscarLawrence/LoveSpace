    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, symbol in enumerate(symbols[:5]):
                print(
                    f"   {symbol['symbol']}: {symbol['baseAsset']}/{symbol['quoteAsset']} - {symbol['status']}"
                )
            ...
            quote_filtered = [
                s for s in trading_symbols if s.get("quoteAsset") in quote_currencies
            ]
            print(f"4. With USDC/USDC/BUSD: {len(quote_filtered)}")

            # Step 5: Filter for SPOT permissions
            spot_filtered = [
                s for s in quote_filtered if "SPOT" in s.get("permissions", [])
            ]
            ...
            if spot_filtered:
                ...
                for symbol in spot_filtered[:10]:
                    print(
                        f"   - {symbol['symbol']}: {symbol['baseAsset']}/{symbol['quoteAsset']}"
                    )
...
def test_ticker_data(symbols):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Ticker data found for {found_count}/{len(symbol_names[:5])} symbols"
            )
