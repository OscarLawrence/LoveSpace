...
def run_basic_backtest():
    ...
    try:
        ...
        print(
            f"  95% Confidence Interval: ({metrics.confidence_interval_95[0]:.4f}, {metrics.confidence_interval_95[1]:.4f})"
        )
        print()
...
def run_walk_forward_validation():
    ...
    optimal_assets = market_discovery.discover_optimal_assets(max_assets=17)
    symbols = [
        asset.symbol for asset in optimal_assets[:8]
    ...
    try:
        ...
        if result.validation_results:
            ...
            print(
                f"Overfitting Score: {val_results.overfitting_score:.3f} (lower is better)"
            )
            print(
                f"Strategy Robustness: {'ROBUST' if val_results.is_robust else 'QUESTIONABLE'}"
            )
            print()
            ...
            for i, period_result in enumerate(val_results.validation_periods):
                print(f"  Period {i + 1}:")
                print(
                    f"    Train Return: {period_result.train_metrics.get('annualized_return', 0):.2%}"
                )
                print(
                    f"    Test Return: {period_result.test_metrics.get('annualized_return', 0):.2%}"
                )
                print(
                    f"    Test Sharpe: {period_result.test_metrics.get('sharpe_ratio', 0):.2f}"
                )
