    ...
    try:
        ...
        for tier_name, symbols in test_tiers.items():
            ...
            for symbol in symbols[:5]:  # Test first 5 to avoid rate limits
                try:
                    ...
                    if len(data) >= 10:
                        ...
                        print(
                            f"   {symbol}: ${current_price:,.4f}, Vol: ${daily_volume:,.0f}/day, "
            ...
            if tier_data["volumes"]:
                tier_data["avg_volume"] = sum(tier_data["volumes"]) / len(
                    tier_data["volumes"]
                )
                tier_data["avg_spread"] = sum(tier_data["price_ranges"]) / len(
                    tier_data["price_ranges"]
                )
            ...
            # Tier summary
            feasible_pct = (
                tier_data["execution_feasible"] / tier_data["symbols_tested"]
            ...
            print("   📊 Summary:")
            print(
                f"      Available Data: {tier_data['symbols_available']}/{tier_data['symbols_tested']}"
            )
            print(f"      Avg Daily Volume: ${tier_data['avg_volume']:,.0f}")
            print(f"      Avg Volatility: {tier_data['avg_spread']:.2%}")
            print(
                f"      Execution Feasible: {tier_data['execution_feasible']}/{tier_data['symbols_tested']} ({feasible_pct:.1%})"
            )
        ...
        for tier_name, data in tier_analysis.items():
            if data["symbols_tested"] > 0:
                ...
                print(
                    f"   Opportunity Score: {opportunity_score:.1f}/10 (higher is better)"
                )
        ...
        best_tier = max(
            tier_analysis.items(),
            key=lambda x: calculate_opportunity_score(x[1])
            - calculate_risk_score(x[1]),
        )

        print(f"   🏆 OPTIMAL TIER: {best_tier[0]}")
        print("   📊 REASONING:")
        print("      • Best risk-adjusted opportunity score")
        print("      • Sufficient liquidity for execution")
        print("      • Manageable volatility and spreads")

        print("\n⚠️  EXPANSION RISKS FOR MICRO CAPS:")
        print("   1. 🚫 Liquidity Risk: Bid-ask spreads can be 5-20%")
        print("   2. 📉 Execution Risk: Cannot fill orders at theoretical prices")
        print("   3. 🎭 Manipulation Risk: Pump and dump schemes common")
        print("   4. 💀 Delisting Risk: Exchanges remove low-volume pairs")
        print("   5. 🔧 Technical Risk: Poor API reliability, data gaps")

        print("\n✅ RECOMMENDED APPROACH:")
        print("   Phase 1: Start with Top 25 (proven liquidity)")
        print("   Phase 2: Add Top 50 after successful Phase 1")
        print("   Phase 3: Expand to Top 100 with enhanced filters")
        print("   Phase 4: Consider micro-caps ONLY with:")
        print("      • Minimum $10K daily volume filter")
        print("      • Maximum 5% spread filter")
        print("      • Real-time delisting monitoring")
        print("      • Smaller position sizes (0.5% max)")
...
def calculate_opportunity_score(tier_data):
    ...
    availability_opportunity = (
        tier_data["symbols_available"] / tier_data["symbols_tested"]
    ) * 10
