...
class ComprehensiveBacktester:
    ...
    def run_regime_backtest(
        self, regime_name: str, regime_config: dict
    ) -> dict[str, Any]:
        """Run backtest for a specific market regime."""
        logger.info(f"\\n{'=' * 60}")
        logger.info(f"TESTING REGIME: {regime_config['name']}")
        logger.info(
            f"Period: {regime_config['start_date'].strftime('%Y-%m-%d')} to {regime_config['end_date'].strftime('%Y-%m-%d')}"
        )
    ...
    def run_comprehensive_analysis(self) -> dict[str, Any]:
        ...
        logger.info(
            "Testing correlation breakdown strategy across multiple market regimes"
        )
        logger.info("=" * 80)
    ...
    def _generate_summary(
        self, regime_results: dict, validation_results: dict
    ) -> dict[str, Any]:
        """Generate comprehensive summary of all results."""
        successful_regimes = [
            r for r in regime_results.values() if r.get("success", False)
        ]
        ...
        avg_return = sum(
            r.get("annualized_return", 0) for r in successful_regimes
        ) / len(successful_regimes)
        avg_sharpe = sum(r.get("sharpe_ratio", 0) for r in successful_regimes) / len(
            successful_regimes
        )
        worst_drawdown = min(r.get("max_drawdown", 0) for r in successful_regimes)
        significant_results = sum(
            1
            for r in successful_regimes
            if r.get("is_statistically_significant", False)
        )
        ...
        return {
            ...
            "validation_robust": validation_results.get("is_robust", False)
            if validation_results.get("success")
            else False,
            "recommendation": self._generate_recommendation(
                status, avg_return, avg_sharpe, worst_drawdown, validation_results
            ),
        }
...
def print_executive_summary(results: dict[str, Any]):
    ...
    print(
        f"   • Statistically Significant: {summary.get('statistically_significant_regimes', 0)}/{summary.get('successful_regimes', 0)}"
    )
    ...
    for regime_name, result in regime_results.items():
        if result.get("success"):
            ...
            print(
                f"   {status_icon} {result.get('market_condition', 'Unknown')}: "
