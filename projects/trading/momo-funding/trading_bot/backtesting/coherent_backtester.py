...
class BacktestResult:
    ...
    validation_results: WalkForwardResults | None = None
...
class CoherentBacktester:
    ...
    def run_backtest(self, config: BacktestConfig) -> BacktestResult:
        ...
        logger.info(
            f"Backtest complete. Total trades: {len(trades)}, "
    ...
    def run_walk_forward_validation(
        ...
        if validation_results.validation_periods:
            ...
            best_result = max(
                validation_results.validation_periods,
    ...
    def _load_historical_data(
        ...
        for symbol in config.symbols:
            ...
            if data and data.quality_metrics.is_acceptable(
                0.60
            ):  # Accept 60%+ for crypto
                data_series[symbol] = data
                logger.info(f"Loaded {len(data.data)} data points for {symbol}")
            elif data:
                logger.warning(
                    f"Insufficient data quality for {symbol}: {data.quality_metrics.quality_score:.3f}"
                )
            else:
    ...
    def _create_combined_dataframe(
        self, data_series: dict[str, AssetDataSeries]
    ) -> pd.DataFrame:
    ...
    def _simulate_trading_strategy(
        self, config: BacktestConfig, data_series: dict[str, AssetDataSeries]
        ...
        for timestamp in all_timestamps:
            ...
            for symbol, series in data_series.items():
                ...
                if closest_data and min_time_diff <= timedelta(
                    hours=2
                ):  # Max 2 hour tolerance
            ...
            opportunities = self.correlation_detector.find_best_opportunities(
                market_data
            )
            ...
            for opportunity in opportunities:
                if self._should_enter_trade(opportunity, current_prices, timestamp):
                    trade = self._execute_trade_entry(
                        opportunity, current_prices, timestamp, config
                    )
                    if trade:
                        trades.append(trade)
    ...
    def _should_enter_trade(
        ...
        if (
            opportunity.pair1 in self.current_positions
            or opportunity.pair2 in self.current_positions
        ):
            return False
    ...
    def _execute_trade_entry(
        ...
        )  # Approximate volatility from risk level
...
def create_coherent_backtester(cache_dir: str | None = None) -> CoherentBacktester:
