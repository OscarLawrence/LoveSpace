...
class ExecutionResult:
    ...
    def fill_rate(self) -> float:
        ...
        return (
            self.total_filled_qty / self.original_request.quantity
...
class LiquidityModel:
    ...
    def get_market_conditions(
        self, symbol: str, base_price: float, volume_24h: float, volatility: float
    ) -> MarketConditions:
        """
...
class SlippageModel:
    ...
    def calculate_slippage(
        self, order_size_usd: float, market_conditions: MarketConditions, symbol: str
    ) -> float:
        """
        ...
        liquidity_multiplier = liquidity_adj / max(
            0.1, market_conditions.liquidity_score
        )
        ...
        slippage_bps = (
            base_slippage * liquidity_multiplier * volatility_multiplier * 10000
        )
...
class MarketSimulator:
    ...
    def execute_order(
        ...
        execution_delay_ms = self.latency_model.calculate_execution_delay(
            market_conditions
        )
        ...
        market_impact_bps = min(
            slippage_bps * 0.5, 100
        )  # Market impact is typically less than slippage
    ...
    def _determine_execution_price(
        self, order: OrderRequest, market_conditions: MarketConditions, delay_ms: float
    ) -> float | None:
        ...
        if order.order_type == OrderType.LIMIT:
            ...
            if order.side == OrderSide.BUY:
                ...
                return (
                    order.limit_price
                    if market_conditions.ask <= order.limit_price
                    else None
                )
            # Sell limit executes if market bid >= limit price
            return (
                order.limit_price
                if market_conditions.bid >= order.limit_price
                else None
            )
    ...
    def _determine_fill_quantity(
        ...
        if order_size_usd > 100000:
            ...
            partial_fill_prob = min(0.5, order_size_usd / 1000000) * (
                1 - market_conditions.liquidity_score
            )
