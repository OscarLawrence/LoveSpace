...
class OHLCV:
    ...
    def __post_init__(self):
        ...
        if not (
            ...
            raise ValueError(
                f"Invalid OHLCV data: O={self.open}, H={self.high}, L={self.low}, C={self.close}"
            )
...
class BinanceDataSource(DataSource):
    ...
    def __init__(self):
        ...
        self.connector = (
            create_binance_connector() if create_binance_connector else None
        )
    ...
    def fetch_historical_data(
        ...
        try:
            ...
            limit = min(
                1000, total_minutes // interval_minutes
            ...
            logger.info(
                f"Fetched {len(ohlcv_data)} data points for {symbol} {timeframe}"
            )
            return ohlcv_data
...
class HistoricalDataEngine:
    ...
    def get_historical_data(
        ...
        if cached_data and cached_data.quality_metrics.is_acceptable(
        ...
        if validate_quality and not quality_metrics.is_acceptable(
            0.60
        ):  # Accept 60%+ quality for crypto
            logger.warning(
                f"Data quality insufficient for {symbol}: {quality_metrics.quality_score:.3f}"
            )
            return None
    ...
    def _calculate_quality_score(
        ...
        price_anomaly_penalty = min(
            1.0, price_anomalies / (total_points * 0.001)
        )  # Penalize >0.1% anomalies
        volume_anomaly_penalty = min(
            1.0, volume_anomalies / (total_points * 0.01)
        )  # Penalize >1% anomalies
        anomaly_score = 1.0 - (
            price_anomaly_penalty * 0.7 + volume_anomaly_penalty * 0.3
        )
...
def create_historical_data_engine(cache_dir: str | None = None) -> HistoricalDataEngine:
