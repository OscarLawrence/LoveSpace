...
class ParameterOptimizer:
    ...
    def get_test_symbols(self, max_symbols: int = 8) -> list[str]:
        ...
        try:
            optimal_assets = self.market_discovery.discover_optimal_assets(
                max_assets=15
            )
    ...
    def test_parameter_combination(
        self, params: dict[str, float], test_period_days: int = 90
    ) -> dict[str, Any]:
    ...
    def run_systematic_optimization(
        ...
        for i, combination in enumerate(all_combinations[:max_combinations]):
            ...
            print(
                f"\\rTesting {i + 1}/{min(len(all_combinations), max_combinations)}: "
    ...
    def run_timeframe_analysis(self) -> dict[str, Any]:
        ...
        for timeframe in timeframes:
            ...
            try:
                ...
                print(
                    f"  {timeframe}: {metrics.total_trades} trades, "
    ...
    def analyze_results(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        results_with_trades = [
            r for r in successful_results if r.get("total_trades", 0) > 0
        ]

        if not results_with_trades:
            # Even with parameter optimization, no trades found
            best_score = max(
                successful_results, key=lambda x: x.get("optimization_score", -999)
            )
            return {
                "status": "NO_TRADES",
        ...
        best_result = max(
            results_with_trades, key=lambda x: x.get("optimization_score", 0)
        )
        most_trades = max(results_with_trades, key=lambda x: x.get("total_trades", 0))
        best_return = max(
            results_with_trades, key=lambda x: x.get("annualized_return", -999)
        )

        # Parameter sensitivity analysis
        correlation_values = [
            r["params"]["correlation_threshold"] for r in results_with_trades
        ]
        ...
        analysis = {
            ...
            "parameter_insights": {
                ...
                "avg_correlation_threshold": sum(correlation_values)
    ...
    def _generate_optimization_recommendation(
        self, results_with_trades: list[dict]
    ) -> str:
        ...
        avg_return = sum(
            r.get("annualized_return", 0) for r in results_with_trades
        ) / len(results_with_trades)
        avg_trades = sum(r.get("total_trades", 0) for r in results_with_trades) / len(
            results_with_trades
        )
    ...
    def print_optimization_summary(
        self, results: list[dict], analysis: dict, timeframe_results: dict = None
    ):
        ...
        if analysis["status"] == "SUCCESS":
            ...
            print(
                f"   • Optimal Correlation Range: {insights['optimal_correlation_range']}"
            )
            print(
                f"   • Optimal Confidence Range: {insights['optimal_confidence_range']}"
            )
            ...
            print(
                f"   • Tested {analysis['total_combinations_tested']} parameter combinations"
            )
        ...
        if timeframe_results:
            ...
            for tf, result in timeframe_results.items():
                if result.get("success"):
                    print(
                        f"   • {tf}: {result['total_trades']} trades, {result['annualized_return']:.1%} return"
                    )
                else:
                    print(f"   • {tf}: Failed")
...
def save_optimization_results(
    results: list[dict], analysis: dict, timeframe_results: dict = None
):
    ...
    full_results = {
        ...
        "summary": {
            ...
            "tests_with_trades": len(
                [r for r in results if r.get("total_trades", 0) > 0]
            ),
        },
    }
