...
sys.path.insert(
    ...
    str(
        ...
        / "MomoAI/projects/coherence/formal_contracts"
    ),
)
...
class CorrelationBreakdownDetector:
    ...
    @coherence_contract(
        ...
        ensures=[
            "len(result) == len(prices) - 1",
            "all(isinstance(r, float) for r in result)",
        ],
    ...
    def calculate_correlation(
        self, x_returns: list[float], y_returns: list[float]
    ) -> float:
        ...
        numerator = sum(
            (x_returns[i] - mean_x) * (y_returns[i] - mean_y) for i in range(n)
        )
    ...
    def detect_breakdown(
        self, btc_data: list[MarketData], eth_data: list[MarketData]
    ) -> TradingSignal | None:
        """
        ...
        current_correlation = self.calculate_correlation(
            recent_btc_returns, recent_eth_returns
        )
        historical_correlation = self.calculate_correlation(
            hist_btc_returns, hist_eth_returns
        )
...
def create_correlation_detector(
