...
"""
    ...
    def _extract_function_with_sphinx(
        self, func_obj: Any, name: str, file_path: Path
    ) -> Function | None:
        ...
        try:
            ...
            return Function(
                ...
                line_number=func_obj.__code__.co_firstlineno
    ...
    def _extract_class_with_sphinx(
        self, cls_obj: Any, name: str, file_path: Path
    ) -> Class | None:
        """Extract class with enhanced documentation using Sphinx."""
        try:
            # Get class hierarchy
            mro = [
                cls.__name__ for cls in cls_obj.__mro__[1:] if cls.__name__ != "object"
            ]

            # Extract methods with signatures
            methods = []
            for method_name, method_obj in inspect.getmembers(
                cls_obj, inspect.isfunction
            ):
                try:
    ...
    def _extract_enhanced_patterns(
        self, tree: ast.AST, file_path: Path, module: Any
    ) -> list[Pattern]:
    ...
    def _classify_function_domain(
        self, name: str, docstring: str, decorators: list[str]
    ) -> str:
        ...
        if any(
            ...
            for keyword in ["save", "load", "create", "update", "delete", "query"]
        ):
            return "database"
    ...
    def _classify_class_domain(
        self, name: str, docstring: str, methods: list[str]
    ) -> str:
        """Classify class domain based on methods and context."""
        method_names = " ".join(methods).lower()

        if any(
            keyword in method_names
            for keyword in ["save", "load", "create", "update", "delete"]
        ):
            return "model"
        elif any(
            keyword in method_names
            for keyword in ["get", "post", "put", "delete", "route"]
        ):
            return "api"
        elif any(keyword in method_names for keyword in ["test", "setup", "teardown"]):
            return "testing"
        elif any(
            keyword in method_names for keyword in ["parse", "extract", "analyze"]
        ):
            return "parser"
