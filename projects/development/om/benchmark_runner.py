...
def main():
    ...
    if summary["pass_rate"] < 0.8:
        quality_issues.append(
            f"Low pass rate: {summary['pass_rate']:.1%} (target: ≥80%)"
        )
    if summary["avg_precision"] < 0.6:
        quality_issues.append(
            f"Low precision: {summary['avg_precision']:.3f} (target: ≥0.6)"
        )
    ...
    if report["failed_queries"]:
        ...
        for i, failed in enumerate(report["failed_queries"][:5], 1):  # Show first 5
            ...
            print(
                f"      Precision: {failed['precision']:.3f}, Recall: {failed['recall']:.3f}"
            )
            if failed["errors"]:
