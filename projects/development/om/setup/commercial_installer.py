...
class CommercialInstaller:
    ...
    def install(self, license_key: str | None = None) -> bool:
    ...
    def _check_system_requirements(self) -> bool:
        ...
        if self.python_version < self.required_python:
            ...
            print(
                f"   Current version: {self.python_version[0]}.{self.python_version[1]}"
            )
            return False
        ...
        if free_space < required_space:
            print(
                f"❌ Insufficient disk space. Required: 500MB, Available: {free_space // 1024 // 1024}MB"
            )
            return False
...
"""
    ...
    def _remove_shell_integration(self) -> None:
        """Remove shell integration."""
        script_paths = [self.install_dir / "om", self.install_dir / "om.bat"]
...
def main():
    ...
    parser.add_argument(
        "--uninstall", action="store_true", help="Uninstall OM Commercial"
    )
