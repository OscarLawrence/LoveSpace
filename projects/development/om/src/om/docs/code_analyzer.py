...
class CodeAnalyzer:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    elements.append(
                        self._analyze_async_function(node, file_path, source)
                    )
    ...
    def _analyze_class(
        self, node: ast.ClassDef, file_path: Path, source: str
    ) -> CodeElement:
    ...
    def _analyze_async_function(
        self, node: ast.AsyncFunctionDef, file_path: Path, source: str
    ) -> CodeElement:
