...
class DocumentationBuilder:
    ...
    def __init__(
        self, source_dir: Path, output_dir: Path, project_name: str = "Project"
    ):
    ...
    def build_documentation(self, file_patterns: list[str] = None) -> bool:
        ...
        try:
            ...
            success = self.generator.generate_documentation(
                self.elements, self.project_name
            )
            if not success:
                return False
    ...
    def _analyze_source_files(self, patterns: list[str]):
        ...
        print(
            f"Analyzed {len(self.elements)} code elements from {len(set(e.source_file for e in self.elements))} files"
        )
    ...
    def generate_coverage_report(self) -> dict[str, any]:
        ...
        documented_elements = len(
            [e for e in self.elements if e.docstring and e.docstring.strip()]
        )
...
def batch_parse_with_sphinx(
    source_dir: Path, output_dir: Path = None, project_name: str = "Project"
) -> dict[str, any]:
    ...
    try:
        ...
        return {
            ...
            "functions_extracted": sum(
                1 for e in builder.elements if e.type in ("function", "async_function")
            ),
