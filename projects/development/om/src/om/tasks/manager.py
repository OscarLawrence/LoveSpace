...
class TaskManager:
    ...
    def create_task(self, title: str, description: str, **kwargs) -> Task:
        ...
        task_type, complexity, scopes = self.analyzer.analyze_task_description(
            description
        )
        ...
        # Create metrics
        effort_points = self.analyzer.estimate_effort(
            task_type, complexity, description
        )
        metrics = TaskMetrics(complexity_score=complexity, effort_points=effort_points)
    ...
    def complete_task(self, task_id: str, quality_score: float | None = None) -> Task:
