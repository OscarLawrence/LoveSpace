...
class PreferenceManager:
    ...
    def get_user_preferences(self, context: str = None) -> dict:
        ...
        for row in results:
            ...
            preferences[category][key] = {"value": parsed_value, "weight": weight}
    ...
    def export_preferences(self) -> dict:
        ...
        export_data = {"exported_at": datetime.now().isoformat(), "preferences": {}}
    ...
    def get_preference_stats(self) -> dict:
        ...
        result = self.db.conn.execute(
            "SELECT COUNT(DISTINCT category) FROM preferences"
        ).fetchone()
        ...
        stats["top_preferences"] = [
            {"category": row[0], "key": row[1], "weight": row[2]} for row in results
        ]

        return stats

    def _store_preference(
        self, category: str, key: str, value: Any, weight: float = 1.0
    ):
