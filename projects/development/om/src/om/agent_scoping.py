...
class AutoScoper:
    ...
    def determine_scope(self, context: ScopeContext) -> ScopeResult:
        ...
        if base_command in self.command_scope_map:
            ...
            confidence_scores.update(dict.fromkeys(direct_scopes, 0.9))
            reasoning_parts.append(f"direct_map:{base_command}→{direct_scopes}")

        # 2. Pattern-based detection
        full_text = (
            f"{context.command} {' '.join(context.args)} {context.task_description}"
        )
        ...
        if context.current_module:
            ...
            confidence_scores.update(
                {s: confidence_scores.get(s, 0) + 0.3 for s in module_scopes}
            )
        ...
        if context.command_history:
            ...
            confidence_scores.update(
                {s: confidence_scores.get(s, 0) + 0.2 for s in history_scopes}
            )
...
class ScopedToolProvider:
    ...
    def __init__(self):
        self.scope_commands = {
            ...
            "parsing": ["populate-patterns", "modules list", "workspace status"],
        }
    ...
    def get_scope_for_command(self, command: str) -> str | None:
...
class ScopeManager:
    ...
    def __init__(self, session_file: Path | None = None):
        ...
        self.scope_history: list[
            tuple[str, list[str], float]
        ] = []  # (timestamp, scopes, confidence)
    ...
    def update_scope(
        self, context: ScopeContext, force_scopes: list[str] | None = None
    ) -> ScopeResult:
        ...
        self.scope_history.append(
            (datetime.now().isoformat(), result.scopes.copy(), result.confidence)
        )
    ...
    def suggest_scope_for_command(self, command: str) -> str | None:
