...
class CLIDocumentationGenerator:
    ...
    def _get_param_type_info(self, param: click.Parameter) -> dict[str, Any]:
        ...
        type_info = {"name": param_type.name, "python_type": type(param_type).__name__}
        ...
        elif isinstance(param_type, click.IntRange) or isinstance(
            param_type, click.FloatRange
        ):
        ...
        if hasattr(command, "callback") and command.callback:
            ...
            if docstring:
                ...
                if example_section:
                    example_lines = [
                        line.strip()
                        for line in example_section.group(1).split("\n")
                        if line.strip()
                    ]
                    examples.extend(example_lines)
    ...
    def generate_markdown_reference(
        self, root_command: click.Group, title: str = "CLI Reference"
    ) -> str:
    ...
    def _generate_command_docs(
        self, command_info: dict[str, Any], level: int = 2
    ) -> list[str]:
        ...
        if command_info["options"]:
            ...
            for option in command_info["options"]:
                ...
                if option["secondary_names"]:
                    names += ", " + ", ".join(
                        [f"`{name}`" for name in option["secondary_names"]]
                    )
        ...
        if command_info["scopes"]:
            ...
            lines.append("")
            lines.append(
                f"This command belongs to scopes: {', '.join(command_info['scopes'])}"
            )
            lines.append("")
    ...
    def generate_completion_data(self, root_command: click.Group) -> dict[str, Any]:
        ...
        completion_data = {"commands": {}, "options": {}, "arguments": {}}
