...
class LearningSystem:
    ...
    def suggest_improvements(self, context: str, task_type: str = None) -> list[str]:
        ...
        for pattern, score in top_patterns:
            if score > 8:  # High confidence threshold
                suggestions.append(
                    f"Consider incorporating '{pattern}' based on past success"
                )
    ...
    def get_performance_metrics(self, days: int = 30) -> dict:
        ...
        if result:
            avg_rating, total_tasks, successful_tasks, task_variety, modules_worked = (
                result
            )
    ...
    def recommend_alternatives(
        self, failed_approach: str, context: str = None
    ) -> list[str]:
        ...
        results = self.db.conn.execute(
            query, [context_pattern, context_pattern, failed_pattern]
        ).fetchall()
        ...
        for row in results:
            ...
            if len(output_result) > 20:
                alternatives.append(
                    {
                        "approach": output_result[:100] + "..."
                        if len(output_result) > 100
                        else output_result,
    ...
    def get_learning_stats(self) -> dict:
        ...
        result = self.db.conn.execute(
            "SELECT AVG(user_rating) FROM learning_data WHERE user_rating IS NOT NULL"
        ).fetchone()
        stats["average_rating"] = round(result[0], 2) if result and result[0] else 0

        # Task types covered
        result = self.db.conn.execute(
            "SELECT COUNT(DISTINCT task_type) FROM learning_data"
        ).fetchone()
        stats["task_types"] = result[0] if result else 0

        # Modules with feedback
        result = self.db.conn.execute(
            "SELECT COUNT(DISTINCT module_context) FROM learning_data WHERE module_context IS NOT NULL"
        ).fetchone()
        stats["modules_with_feedback"] = result[0] if result else 0

        # Recent activity (last 7 days)
        result = self.db.conn.execute(
            "SELECT COUNT(*) FROM learning_data WHERE created_at > (CURRENT_TIMESTAMP - INTERVAL 7 DAY)"
        ).fetchone()
