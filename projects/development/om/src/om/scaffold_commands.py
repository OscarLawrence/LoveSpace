...
@click.option(
    ...
    default=["speed", "maintainability"],
    help="Development objectives",
)
    ...
    try:
        ...
        if plan_only:
            ...
            for subsystem in subsystems:
                click.echo(
                    f"  {subsystem.name}: {subsystem.estimated_lines} lines, complexity {subsystem.complexity_score:.2f}"
                )
            ...
            if plan.integration_points:
                ...
                for integration in plan.integration_points:
                    click.echo(
                        f"  {integration.subsystem_a} ↔ {integration.subsystem_b}: {integration.interface_type}"
                    )
...
@click.option(
    "--integration-mode",
    default="auto",
    help="Integration approach with other subsystems",
)
def subsystem(subsystem_name, pattern, integration_mode):
    ...
    try:
        ...
        click.echo(
            f"scaffold:subsystem:{subsystem_name}:pattern:{pattern or 'auto'}:integration:{integration_mode}"
        )
...
def system(requirements, objectives, dry_run):
    ...
    try:
        ...
        if dry_run:
            click.echo(
                f"system:dry_run:subsystems:{len(subsystems)}:total_lines:{plan.estimated_total_lines}"
            )
        ...
        click.echo(
            f"system:generated:subsystems:{len(subsystems)}:total_lines:{plan.estimated_total_lines}"
        )
...
def integration(subsystem_a, subsystem_b, interface_type):
    ...
    try:
        ...
        click.echo(
            f"integration:{subsystem_a}:{subsystem_b}:interface:{interface_type}:status:validated"
        )
