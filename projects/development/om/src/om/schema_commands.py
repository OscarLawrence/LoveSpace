...
@click.option(
    "--format", "output_format", default="json", type=click.Choice(["json", "markdown"])
)
...
def generate(source_file, output, output_format, validate):
    ...
    if output_format == "markdown":
        ...
        content = doc_generator.generate_markdown_docs(
            schemas, f"Types from {source_path.name}"
        )
    else:
...
@click.option(
    "--output-dir", "-o", default="schemas", help="Output directory for schema files"
)
@click.option(
    "--patterns",
    "-p",
        ...
        try:
            ...
            if schemas:
                ...
                if output_format == "markdown":
                    ...
                    content = doc_generator.generate_markdown_docs(
                        schemas, f"Types from {py_file.name}"
                    )
                else:
