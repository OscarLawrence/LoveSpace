...
class SessionContext:
    ...
    context_data: dict
    isolated_workspace_id: str | None
    created_at: datetime
...
class SessionManager:
    ...
    def store_session_context(
        self, project_id: str, context_data: dict, module_name: str = None
    ) -> str:
    ...
    def track_user_decisions(
        self, decision_type: str, choice: str, outcome: str, module_name: str = None
    ):
        ...
        context = json.dumps(
            {"timestamp": datetime.now().isoformat(), "module": module_name}
        )

        self.db.conn.execute(
            query, [decision_type, module_name, context, choice, outcome]
        )

    def get_decision_patterns(
        self, decision_type: str, module_name: str = None
    ) -> list[dict]:
    ...
    def save_session(self, project_id: str, module: str = None) -> dict:
        ...
        context_data = {
            ...
            "timestamp": time.time(),
            "session_metadata": {"pid": os.getpid(), "auto_save": True},
        }
                ...
                for file in files:
                    if file.endswith((".py", ".json", ".md", ".txt")):
                        ...
                        try:
                            ...
                            continue
            return sorted(
                recent_files, key=lambda x: os.path.getmtime(x), reverse=True
            )[:10]
        except Exception:
            return []
