...
def context():
    ...
    try:
        ...
        click.echo(
            f"context:active_modules:{len(context.get('modules', []))} "
...
@click.option(
    "--format",
    "-f",
    type=click.Choice(["dense", "json", "yaml"]),
    default="dense",
    help="Output format",
)
    ...
    try:
        ...
        if format == "dense":
            for result in results:
                click.echo(
                    f"{result['type']}:{result['name']}:{result['relevance']:.2f}"
                )
        else:
            import json
...
def populate_patterns(force, enhanced):
    ...
    try:
        ...
        click.echo(
            f"patterns_populated:{result['count']} "
