...
def create(module_name, task_type):
    ...
    try:
        ...
        click.echo(
            f"isolation_created:{module_name}:{task_type}:env_id:{result['env_id']}"
        )
...
def focus(module_name):
    ...
    try:
        ...
        click.echo(
            f"module_focused:{module_name}:dependencies:{result['dependencies']} "
...
def boundaries(module_name, dependencies, rules):
    ...
    try:
        ...
        result = isolation.set_module_boundaries(
            module_name, list(dependencies), list(rules)
        )
        isolation.close()

        click.echo(
            f"boundaries_set:{module_name}:deps:{len(dependencies)}:rules:{len(rules)}"
        )
...
def inject(module_name, task):
    ...
    try:
        ...
        click.echo(
            f"task_injected:{module_name}:task_id:{result['task_id']} "
