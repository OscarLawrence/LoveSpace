    ...
    for py_file in Path("modules").rglob("*.py"):
        try:
            ...
            if re.search(query, content, re.IGNORECASE):
                matches = re.findall(
                    rf"(?:class|def)\s+(\w*{re.escape(query)}\w*)",
                    content,
                    re.IGNORECASE,
                )
...
@click.argument("query")
@click.option(
    "--auto-parse", "-a", is_flag=True, help="Automatically parse top results"
)
@click.option(
    "--provider",
    "-p",
    type=click.Choice(["auto", "tavily", "serper"]),
    default="auto",
            ...
            for result in results[:2]:  # Parse top 2 results
                try:
                    ...
                    if parsed:
                        ...
                        click.echo(
                            parsed[:500] + "..." if len(parsed) > 500 else parsed
                        )
                        click.echo()
                except:
                    continue
