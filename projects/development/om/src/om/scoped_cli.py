...
class ScopedCLI:
    ...
    def process_command(
        ...
        try:
            ...
            coherence_result = om_integration.validate_before_execution(
                command, locals()
            )
            if om_integration.should_halt_execution(coherence_result):
                click.echo(
                    f"🛑 EXECUTION HALTED: {coherence_result.contradictions or coherence_result.impossibilities}"
                )
                return False
        except ImportError:
        ...
        # Update scope
        if explicit_scopes:
            scope_result = self.scope_manager.update_scope(
                scope_context, explicit_scopes
            )
        ...
        if not self.scope_manager.is_command_in_scope(command):
            ...
            if suggested_scope:
                click.echo(
                    f"scope_error:command_not_in_scope:{command}:suggested_scope:{suggested_scope}"
                )
        ...
        if self.verbose_scope_info:
            click.echo(
                f"scope_info:active:{':'.join(scope_result.scopes)}:confidence:{scope_result.confidence:.2f}"
            )

        return True

    def show_scoped_help(self, scopes: list[str] | None = None):
        """Show help filtered by current or specified scopes."""
        target_scopes = scopes or self.scope_manager.current_scopes
        available_commands = self.scope_manager.tool_provider.filter_commands(
            target_scopes
        )
...
def scope_option(f):
    ...
    f = click.option(
        "--scope",
        multiple=True,
        help="Explicit scope selection (docs, memory, analysis, code, parsing)",
    )(f)
    f = click.option(
        "--auto-scope", is_flag=True, help="Enable automatic scope determination"
    )(f)
...
def apply_scoping(
    ctx: click.Context, scope: tuple, auto_scope: bool, no_scope: bool, scope_info: bool
):
    ...
    return scoped_cli.process_command(
        ctx, command_parts, explicit_scopes=explicit_scopes
    )
...
def auto(task, module, history):
    ...
    context = create_scope_context(
        command="", task=task or "", module=module, history=history_list
    )

    result = scoped_cli.scope_manager.update_scope(context)

    click.echo(
        f"scope:auto_determined:{':'.join(result.scopes)}:confidence:{result.confidence:.2f}"
    )
...
def show():
    ...
    click.echo(
        f"scope:stats:changes:{stats['total_changes']}:confidence:{stats['avg_confidence']:.2f}"
    )
...
def validate_scope(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        if hasattr(f, "_om_scopes"):
            ...
            if current_scopes and not any(
                scope in current_scopes for scope in required_scopes
            ):
                click.echo(
                    f"scope_error:command_requires_scope:{command_name}:required:{':'.join(required_scopes)}"
                )
                return
