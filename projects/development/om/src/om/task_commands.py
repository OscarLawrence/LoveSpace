...
@click.option(
    "--status",
    type=click.Choice(["pending", "in_progress", "blocked", "completed", "cancelled"]),
)
@click.option(
    "--type",
    "task_type",
...
def list(status, task_type, priority, assigned, output_format):
    ...
    if output_format == "json":
        ...
        for task in tasks:
            ...
            click.echo(
                f"task:list:row:{task.id[:8]}:{task.title}:{task.task_type.value}:{task.status.value}:{task.priority.value}:{due_str}"
            )
...
@click.option(
    ...
    "output_format",
    default="detailed",
    type=click.Choice(["detailed", "json", "tree"]),
)
@scoped_command(["memory"])
@validate_scope
    ...
    if output_format == "json":
        ...
        if task.subtasks:
            ...
            for subtask_id in task.subtasks:
                ...
                if subtask:
                    click.echo(
                        f"task:show:subtask:{subtask_id[:8]}:{subtask.title}:{subtask.status.value}"
                    )
...
def dashboard():
    ...
    click.echo(
        f"task:dashboard:productivity:{dashboard_data['productivity_score']:.1f}/10"
    )
    click.echo(
        f"task:dashboard:avg_completion:{dashboard_data['avg_completion_time_minutes']:.0f}min"
    )
    ...
    if dashboard_data["overdue_tasks"]:
        ...
        for task_data in dashboard_data["overdue_tasks"][:5]:
            click.echo(
                f"task:dashboard:overdue:{task_data['id'][:8]}:{task_data['title']}"
            )
