...
def analyze(source_dir, patterns, output, output_format, min_coverage, verbose):
    ...
    # Summary output
    click.echo(
        f"coverage:summary:total:{metrics.total_elements}:documented:{metrics.documented_elements}"
    )
...
@click.option(
    "--min-coverage", type=float, default=80.0, help="Minimum coverage percentage"
)
...
def issues(source_dir, severity, output, limit):
    ...
    # Format output
    lines = [
        f"Documentation Issues - {severity.upper()} ({len(filtered_issues)} shown)"
    ]
    lines.append("=" * 60)
    ...
    if truncated:
        lines.append(
            f"... {len(metrics.issues) - limit} more issues (use --limit to see more)"
        )
