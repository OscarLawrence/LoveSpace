...
class QualityGate:
    ...
    def check_quality_gate(self, metrics: CoverageMetrics) -> tuple[bool, list[str]]:
        ...
        warning_count = sum(
            1 for issue in metrics.issues if issue["severity"] == "warning"
        )

        # Check error threshold
        if error_count > self.config["max_errors"]:
            failures.append(
                f"{error_count} errors exceed maximum {self.config['max_errors']}"
            )

        # Check warning threshold
        if warning_count > self.config["max_warnings"]:
            failures.append(
                f"{warning_count} warnings exceed maximum {self.config['max_warnings']}"
            )
        ...
        if self.config["fail_on_missing_critical"]:
            ...
            if critical_missing:
                failures.append(
                    f"{len(critical_missing)} critical elements lack documentation"
                )

        return len(failures) == 0, failures

    def create_config_file(
        self, output_path: str, custom_config: dict[str, Any] = None
    ):
