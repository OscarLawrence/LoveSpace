...
from .sphinx_auto import CodeAnalyzer, DocumentationBuilder
...
@click.option(
    "--build-html", is_flag=True, help="Build HTML documentation after generation"
)
...
def generate(source, output, patterns, build_html, verbose):
    ...
    click.echo(
        f"docs:analysis:files:{stats['files_processed']}:elements:{stats['elements_found']}"
    )
...
@click.option(
    "--format", "output_format", default="json", type=click.Choice(["json", "text"])
)
@scoped_command(["docs"])
@validate_scope
def analyze(source_dir, output, output_format):
    ...
    if output_format == "json":
        ...
        output_lines = [
            f"Documentation Analysis Results ({len(all_elements)} elements)"
        ]
...
def build(docs_dir, output_format, clean):
    ...
    try:
        ...
        result = subprocess.run(
            ["make", output_format], cwd=docs_path, capture_output=True, text=True
        )
