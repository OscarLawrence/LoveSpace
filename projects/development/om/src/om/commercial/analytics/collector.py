...
class AnalyticsCollector:
    ...
    def record_command_usage(
        self, command: str, duration_ms: int, success: bool
    ) -> None:
    ...
    def get_usage_summary(self, days: int = 7) -> dict[str, Any]:
        ...
        success_rate = sum(1 for m in recent_usage if m["success"]) / max(
            total_commands, 1
        )
        avg_duration = sum(m["duration_ms"] for m in recent_usage) / max(
            total_commands, 1
        )
