...
class CodebaseAnalyzer:
                ...
                high_impact.append(
                    {
                        ...
                        "impact_score": dep_graph.out_degree(node)
                        * (dep_graph.in_degree(node) + 1),
                    }
                )
                ...
                pattern_stats.append(
                    {
                        ...
                        "context": pattern.usage_context[:50]
        ...
        for module in modules.values():
            ...
            if docstring_coverage < 0.5:
                gaps["missing_docstrings"].append(
                    {"module": module.name, "coverage": docstring_coverage}
                )
        ...
        for module in modules.values():
            for import_name in module.imports:
                ...
                for target_module in modules:
                    if import_name in target_module or target_module.endswith(
                        import_name
                    ):
    ...
    def _calculate_modularity(self, clusters: list[list[str]]) -> float:
        ...
        size_variance = sum(
            (size - total_modules / len(clusters)) ** 2 for size in cluster_sizes
        )
    ...
    def _find_test_file(self, module_path: str) -> str | None:
