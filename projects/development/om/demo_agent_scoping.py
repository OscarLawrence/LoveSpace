...
def demo_cognitive_load_reduction():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        context = create_scope_context(
            command=scenario["command"], task=scenario["task"]
        )
        ...
        print(
            f"AVAILABLE COMMANDS: {len(scoped_commands)} (vs {len(all_commands)} total)"
        )
        print(f"COGNITIVE LOAD: {'LOW' if len(scoped_commands) <= 10 else 'MEDIUM'}")
        print(f"COMMANDS: {', '.join(scoped_commands)}")
        print(
            f"REDUCTION: {((len(all_commands) - len(scoped_commands)) / len(all_commands) * 100):.1f}%\n"
        )
...
def demo_scope_switching():
    ...
    for step, (command, description) in enumerate(workflow, 1):
        ...
        context = create_scope_context(
            command=command, history=[cmd for cmd, _ in workflow[: step - 1]]
        )
        ...
        print(
            f"SCOPE: {', '.join(result.scopes)} (confidence: {result.confidence:.2f})"
        )
