...
class BenchmarkReporter:
    ...
    def generate_report(self, results: list[BenchmarkResult]) -> dict:
        ...
        avg_precision = (
            sum(r.precision for r in results) / total_tests if total_tests > 0 else 0
        )
        avg_recall = (
            sum(r.recall for r in results) / total_tests if total_tests > 0 else 0
        )
        avg_similarity = (
            sum(r.avg_similarity for r in results) / total_tests
            if total_tests > 0
            else 0
        )
