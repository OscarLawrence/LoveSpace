...
class BenchmarkEvaluator:
    ...
    def calculate_precision(
        ...
        if total_found == 0:
            return (
                1.0
                if not query_spec.expected_functions
                and not query_spec.expected_patterns
                else 0.0
            )
        ...
        for func in functions_found:
            if any(
                expected in func.lower() for expected in query_spec.expected_functions
            ):
                relevant_found += 1
        ...
        for pattern in patterns_found:
            if any(
                expected in pattern.lower() for expected in query_spec.expected_patterns
            ):
                relevant_found += 1
        ...
        for doc in docs_found:
            if any(
                keyword in doc.lower() for keyword in query_spec.expected_docs_keywords
            ):
                relevant_found += 1
    ...
    def calculate_recall(
        ...
        total_expected = len(query_spec.expected_functions) + len(
            query_spec.expected_patterns
        )
        if total_expected == 0:
            return 1.0
