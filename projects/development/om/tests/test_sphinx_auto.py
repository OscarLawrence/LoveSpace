...
class TestDoclessArchitecture:
    ...
    def test_infer_purpose_from_name(self):
        ...
        assert "Retrieves user data" in self.extractor._infer_purpose_from_name(
            "get_user_data"
        )
        assert "Creates new session" in self.extractor._infer_purpose_from_name(
            "create_new_session"
        )
        assert "Validates input" in self.extractor._infer_purpose_from_name(
            "validate_input"
        )
