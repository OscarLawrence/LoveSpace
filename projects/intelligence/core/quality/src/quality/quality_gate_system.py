...
class QualityGateSystem:
    ...
    def file_size_gate(self, directory: str = ".") -> QualityGateResult:
        ...
        score = (
            max(0.0, (total_files - violation_count) / total_files)
            if total_files > 0
            else 1.0
        )
        ...
        if violation_count > 0:
            recommendations.append(
                f"Refactor {violation_count} large files into smaller modules"
            )
            recommendations.append(
                "Consider extracting classes or functions into separate files"
            )
    ...
    def complexity_gate(self, directory: str = ".") -> QualityGateResult:
        ...
        score = (
            max(0.0, (total_functions - violation_count) / total_functions)
    ...
    def documentation_gate(self, directory: str = ".") -> QualityGateResult:
        ...
        passed = (
            avg_documentation_ratio >= self.quality_standards["min_documentation_ratio"]
        )
        ...
        if not passed:
            recommendations.append(
                "Add more docstrings and comments to improve documentation"
            )
    ...
    def structure_gate(self, directory: str = ".") -> QualityGateResult:
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if file.endswith(".py"):
                    file_path = os.path.join(root, file)
                    complexity_metrics = self.code_analyzer.analyze_function_complexity(
                        file_path
                    )
                    ...
                    for metric in complexity_metrics:
                        if (
                            ...
                            > self.quality_standards["max_function_parameters"]
                        ):
                            violations.append(
                                f"{file_path}:{metric.function_name} has {metric.parameter_count} parameters"
                            )

                        if (
                            metric.nesting_depth
                            > self.quality_standards["max_nesting_depth"]
                        ):
                            violations.append(
                                f"{file_path}:{metric.function_name} has nesting depth {metric.nesting_depth}"
                            )
        ...
        if violation_count > 0:
            recommendations.append(
                "Reduce function parameter counts by using data classes or configuration objects"
            )
            recommendations.append(
    ...
    def save_quality_report(
        self, directory: str = ".", output_file: str = "quality_report.json"
    ):
