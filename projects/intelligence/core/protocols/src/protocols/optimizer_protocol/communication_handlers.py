...
from .data_models import MessageType, OptimizerMessage, Priority
...
class CommunicationHandlers:
    ...
    async def send_performance_update(
        ...
        await self.message_manager.broadcast_message(
            MessageType.PERFORMANCE_UPDATE, payload, Priority.MEDIUM
        )

    async def send_strategy_change(
        self,
        old_strategy: str,
        new_strategy: str,
        reason: str,
        ...
        await self.message_manager.broadcast_message(
            MessageType.STRATEGY_CHANGE, payload, Priority.HIGH
        )

    async def request_optimization(
        self, target_agent: str, optimization_type: str, parameters: dict[str, Any]
    ) -> str:
    ...
    async def send_feedback_report(
        ...
        await self.message_manager.broadcast_message(
            MessageType.FEEDBACK_REPORT, payload, Priority.MEDIUM
        )
    ...
    async def send_heartbeat(self) -> None:
        ...
        await self.message_manager.broadcast_message(
            MessageType.HEARTBEAT, payload, Priority.LOW
        )
