...
class MetricsCollector:
        ...
        with self.lock:
            if series_key not in self.metrics_buffer:
                self.metrics_buffer[series_key] = MetricSeries(
                    metric_type=metric_type, tags=tags
                )
    ...
    def get_metric_series(
        self, metric_type: MetricType, tags: dict[str, str] | None = None
    ) -> MetricSeries | None:
    ...
    def get_metric_statistics(
        ...
        windowed_points = [p for p in recent_points if p.timestamp >= cutoff_time]
    ...
    def clear_metrics(self, metric_type: MetricType | None = None) -> None:
    ...
    def get_collection_summary(self) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            total_points = sum(
                len(series.points) for series in self.metrics_buffer.values()
            )
        ...
        return {
            ...
            "buffer_utilization": total_points / (series_count * self.buffer_size)
            if series_count > 0
            else 0,
