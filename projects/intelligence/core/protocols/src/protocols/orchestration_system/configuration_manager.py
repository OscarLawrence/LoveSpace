...
class ConfigurationManager:
    ...
    def __init__(
        self, services: dict[str, ServiceInfo], system_configuration: dict[str, Any]
    ):
    ...
    async def _analyze_and_adjust_service(self, service: ServiceInfo) -> None:
        ...
        if cpu_usage > 0.8:
            current_cpu = float(
                service.configuration.get("cpu_limit", "1.0").rstrip("GB")
            )
    ...
    async def handle_service_scaling(self) -> None:
        ...
        for service_type, instances in service_types.items():
            healthy_instances = [
                s for s in instances if s.status == ServiceStatus.HEALTHY
            ]
            ...
            avg_cpu = sum(
                s.performance_metrics.get("cpu_usage", 0.5) for s in healthy_instances
            ) / len(healthy_instances)
            avg_memory = sum(
                s.performance_metrics.get("memory_usage", 0.5)
                for s in healthy_instances
            ) / len(healthy_instances)
    ...
    async def _scale_down_service(self, service_type: str) -> None:
        ...
        if len(instances) > 1:
            ...
            least_loaded = min(
                instances, key=lambda s: s.performance_metrics.get("cpu_usage", 0.5)
            )
