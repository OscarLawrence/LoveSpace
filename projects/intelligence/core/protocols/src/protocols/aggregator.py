...
class AggregationRule:
    ...
    percentile_value: float | None = None  # for percentile aggregation
...
class MetricsAggregator:
    ...
    def _apply_aggregation_rule(
        ...
        for series_key, series in metrics_data.items():
            ...
            if rule.tags_filter:
                if not all(
                    series.tags.get(k) == v for k, v in rule.tags_filter.items()
                ):
                    continue
        ...
        for series in matching_series:
            windowed_points = [
                point for point in series.points if point.timestamp >= cutoff_time
            ]
    ...
    def _calculate_aggregation(
        self, rule: AggregationRule, points: list[MetricPoint], current_time: float
    ) -> dict[str, Any]:
    ...
    def _calculate_efficiency_ratio(
        ...
        resource_values = self._get_recent_values(
            metrics_data, MetricType.RESOURCE_USAGE
        )
    ...
    def _calculate_quality_index(
        ...
        return {
            ...
            "coherence_adjustment": np.mean(coherence_values)
            if coherence_values
            else None,
    ...
    def _calculate_stability_score(
        ...
        overall_stability = np.mean(
            [m["stability"] for m in stability_metrics.values()]
        )
