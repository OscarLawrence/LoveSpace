...
from .collector import MetricSeries, MetricType
...
class MetricsAnalyzer:
    ...
    def analyze_performance_patterns(
        self, metrics_data: dict[str, MetricSeries]
    ) -> dict[str, Any]:
    ...
    def _detect_cycles(self, values: list[float]) -> dict[str, Any] | None:
    ...
    def _detect_anomalies(
        ...
        for series_key, series in metrics_data.items():
            ...
            for i, (z_score, value, timestamp) in enumerate(
                zip(z_scores, values, timestamps, strict=False)
            ):
            ...
            for i, (value, timestamp) in enumerate(
                zip(values, timestamps, strict=False)
            ):
    ...
    def _calculate_correlation(
        self, series1: MetricSeries, series2: MetricSeries
    ) -> dict[str, Any] | None:
        ...
        values1 = [
            p.value for p in series1.points if min_time <= p.timestamp <= max_time
        ]
        values2 = [
            p.value for p in series2.points if min_time <= p.timestamp <= max_time
        ]
    ...
    def _generate_insights(
        self, metrics_data: dict[str, MetricSeries], analysis_results: dict[str, Any]
        ...
        for pair_key, correlation in analysis_results["correlations"].items():
            if (
                ...
                and correlation["significance"] == "significant"
            ):
                insight = PerformanceInsight(
