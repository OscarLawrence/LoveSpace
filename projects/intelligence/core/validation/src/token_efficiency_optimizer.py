...
class TokenEfficiencyOptimizer:
    ...
    def estimate_token_cost(self, request: str) -> dict[str, Any]:
        ...
        if (
            len(
                re.findall(
                    r"\b(?:create|delete|modify|update)\b", request, re.IGNORECASE
                )
            )
            > 3
        ):
        ...
        if (
            len(re.findall(r"\b(?:file|directory|folder)\b", request, re.IGNORECASE))
            > 5
        ):
    ...
    def suggest_batch_operations(self, request: str) -> list[str]:
        ...
        file_ops = re.findall(
            r"(create|delete|update|modify)\s+(?:file\s+)?([^\s,]+)",
            request,
            re.IGNORECASE,
        )
        ...
        package_ops = re.findall(
            r"(install|add|remove)\s+(?:package\s+)?([^\s,]+)", request, re.IGNORECASE
        )
    ...
    def _detect_ambiguity(self, request: str) -> list[str]:
        ...
        for issue_type, pattern in self.clarity_patterns.items():
            ...
            if matches:
                issues.append(
                    f"Ambiguous {issue_type}: {', '.join(list(set(matches))[:3])}"
                )
    ...
    def _suggest_specificity(self, request: str) -> list[str]:
        ...
        if re.search(r"\bfiles?\b", request, re.IGNORECASE) and not re.search(
            r"/\w+", request
        ):
        ...
        if re.search(r"\blatest\b", request, re.IGNORECASE):
            suggestions.append(
                "Consider specifying exact version numbers instead of 'latest'"
            )
    ...
    def generate_optimization_report(self, results: list[OptimizationResult]) -> dict:
        ...
        return {
            ...
            "average_savings_per_request": total_savings / len(results)
            if results
            else 0,
            "top_optimization_types": self._get_top_improvement_types(results),
        }

    def _get_top_improvement_types(
    ...
    def analyze_request_patterns(self, requests: list[str]) -> dict[str, Any]:
        ...
        for op in operations:
            patterns["repeated_operations"][op] = (
                patterns["repeated_operations"].get(op, 0) + 1
            )

        # Identify file type patterns
        file_extensions = re.findall(r"\.(\w+)", " ".join(requests))
        for ext in file_extensions:
            patterns["common_file_types"][ext] = (
                patterns["common_file_types"].get(ext, 0) + 1
            )

        # Find batch opportunities
        if patterns["repeated_operations"]:
            most_common = max(
                patterns["repeated_operations"].items(), key=lambda x: x[1]
            )
            if most_common[1] > 2:
    ...
    def optimize_for_context_efficiency(
        self, request: str, available_context: dict
    ) -> str:
        ...
        if "python_env" in available_context:
            ...
            optimized = re.sub(
                rf"\b{env_name}\s+environment\b", "", optimized, flags=re.IGNORECASE
            )
        ...
        if "project_name" in available_context:
            ...
            optimized = re.sub(
                rf"\b{project_name}\s+project\b", "", optimized, flags=re.IGNORECASE
            )
    ...
    def calculate_complexity_score(self, request: str) -> float:
        ...
        complexity_factors = {
            "file_operations": len(
                re.findall(r"\b(create|delete|move|copy)\b", request, re.IGNORECASE)
            ),
            "code_analysis": len(
                re.findall(
                    r"\b(parse|analyze|inspect|validate)\b", request, re.IGNORECASE
                )
            ),
            "installations": len(
                re.findall(r"\b(install|add|remove)\b", request, re.IGNORECASE)
            ),
            "configurations": len(
                re.findall(r"\b(configure|setup|initialize)\b", request, re.IGNORECASE)
            ),
            "dependencies": len(
                re.findall(r"\b(import|require|dependency)\b", request, re.IGNORECASE)
            ),
        }
    ...
    def generate_efficiency_metrics(
        self, original_request: str, optimized_request: str
    ) -> dict:
        ...
        reduction_percentage = (
            (character_reduction / original_length * 100) if original_length > 0 else 0
        )
        ...
        return {
            ...
            "efficiency_rating": self._get_efficiency_rating(
                reduction_percentage, clarity_score
            ),
        }
    ...
    def _calculate_clarity_score(self, text: str) -> float:
        ...
        has_ambiguous_pronouns = bool(
            re.search(self.clarity_patterns["ambiguous_pronouns"], text)
        )
        has_vague_quantifiers = bool(
            re.search(self.clarity_patterns["vague_quantifiers"], text)
        )
    ...
    def _get_efficiency_rating(
        self, reduction_percentage: float, clarity_score: float
    ) -> str:
