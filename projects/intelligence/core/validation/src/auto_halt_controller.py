...
class AutoHaltController:
    ...
    def __init__(self):
        ...
        self.halt_log = []
        self.halt_patterns = {
            "critical_contradictions": ["delete.*create", "remove.*add", "start.*stop"],
            "resource_violations": [
                "insufficient.*memory",
                "missing.*dependency",
                "unavailable.*service",
    ...
    def should_halt_execution(
        self, coherence_result: CoherenceResult, command: str
    ) -> bool:
        ...
        critical_impossibilities = self._check_critical_impossibilities(
            coherence_result.impossibilities
        )
        if critical_impossibilities:
            self.log_halt_event(
                command,
                f"Critical impossibilities: {critical_impossibilities}",
                coherence_result.score,
                "impossibility",
            )
            return True
        ...
        if coherence_result.score < self.halt_threshold:
            self.log_halt_event(
                command,
                f"Low coherence score: {coherence_result.score}",
                coherence_result.score,
                "low_score",
            )
            return True
    ...
    def generate_halt_reason(
        ...
        if coherence_result.contradictions:
            reasons.append(
                f"Contradictions found: {'; '.join(coherence_result.contradictions[:2])}"
            )

        if coherence_result.impossibilities:
            critical = self._check_critical_impossibilities(
                coherence_result.impossibilities
            )
            if critical:
        ...
        if coherence_result.score < self.halt_threshold:
            reasons.append(
                f"Coherence score too low: {coherence_result.score:.2f} < {self.halt_threshold}"
            )
    ...
    def get_halt_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_halts": len(
                [e for e in self.halt_log if time.time() - e.timestamp < 3600]
            ),
        }
