...
class CoherenceScorer:
    ...
    def calculate_coherence_score(
        ...
        confidence_adjustment = (confidence_level - 0.5) * self.scoring_weights[
        ...
        return CoherenceScore(
            ...
            breakdown={
                ...
                "contradiction_impact": -contradiction_penalty
                * self.scoring_weights["contradiction_penalty"],
                "completeness_impact": completeness_bonus
                * self.scoring_weights["completeness_bonus"],
                "pattern_impact": pattern_consistency
                * self.scoring_weights["pattern_consistency"],
                "confidence_impact": confidence_adjustment,
            },
        )

    def validate_coherence_threshold(
        self, coherence_score: CoherenceScore, threshold: float = 0.7
    ) -> ValidationResult:
    ...
    def _calculate_pattern_consistency(self, pattern_matches: list) -> float:
        ...
        total_confidence = sum(
            getattr(match, "match_confidence", 0.5) for match in pattern_matches
        )
    ...
    def _generate_coherence_recommendations(
        self, coherence_score: CoherenceScore
    ) -> list[str]:
    ...
    def compare_coherence_scores(
        self, score_a: CoherenceScore, score_b: CoherenceScore
    ) -> dict[str, float]:
        """Compare two coherence scores"""

        return {
            "overall_difference": score_b.overall_score - score_a.overall_score,
            "contradiction_difference": score_a.contradiction_penalty
            - score_b.contradiction_penalty,
            "completeness_difference": score_b.completeness_bonus
            - score_a.completeness_bonus,
            "pattern_difference": score_b.pattern_consistency
            - score_a.pattern_consistency,
            "confidence_difference": score_b.confidence_level
            - score_a.confidence_level,
        }
