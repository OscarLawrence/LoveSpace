...
class ContextValidator:
    ...
    def validate_context(
        ...
        present_elements = self._extract_present_elements(
            context, required_elements | optional_elements
        )
        ...
        critical_missing = self._identify_critical_missing(
            missing_elements, operation_type
        )
    ...
    def check_prerequisites(
        ...
        if completeness.completeness_score >= self.completeness_thresholds["critical"]:
            ...
            completeness.completeness_score >= self.completeness_thresholds["important"]
        ):
            return ValidationResult(
                ...
                score=completeness.completeness_score,
                recommendations=[
                    f"Consider providing: {', '.join(completeness.missing_elements)}"
                ],
            )
        else:
    ...
    def _detect_operation_type(self, request: str) -> str | None:
    ...
    def _extract_present_elements(
        self, context: dict[str, Any], all_elements: set[str]
    ) -> set[str]:
    ...
    def _calculate_completeness_score(
        self, required: set[str], present: set[str], optional: set[str]
    ) -> float:
    ...
    def _identify_critical_missing(
        self, missing: set[str], operation_type: str
    ) -> list[str]:
    ...
    def _generate_suggestion(self, missing_element: str, request: str) -> str:
        ...
        return suggestions_map.get(
            missing_element, f"Provide {missing_element.replace('_', ' ')}"
        )
