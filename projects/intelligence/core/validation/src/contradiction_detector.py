...
class ContradictionDetector:
    ...
    def _check_sentence_pair(
        self, sent_a: str, sent_b: str, line_a: int, line_b: int
    ...
    def _check_boolean_patterns(
        ...
        for pos_pattern, neg_pattern in self.contradiction_patterns["boolean"]:
            if re.search(pos_pattern, sent_a) and re.search(neg_pattern, sent_b):
                return (
                    "boolean_match",
                    self.confidence_weights["boolean_contradiction"],
                )
        ...
        for word_a, word_b in self.contradiction_patterns["semantic"]:
            if word_a in sent_a and word_b in sent_b:
                ...
                if self._similar_context(sent_a, sent_b, word_a, word_b):
                    return (
                        "semantic_match",
                        self.confidence_weights["semantic_opposition"],
                    )

        return None

    def _similar_context(
        self, sent_a: str, sent_b: str, word_a: str, word_b: str
    ) -> bool:
