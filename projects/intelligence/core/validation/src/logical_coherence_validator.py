...
class LogicalCoherenceValidator:
    ...
    def _detect_direct_contradictions(self, text: str) -> list[CoherenceIssue]:
        ...
        negation_patterns = [
            (
                r"(\w+)\s+(?:is|are)\s+(\w+).*(?:is|are)\s+not\s+\2",
                "Contradictory statements about {}",
            ),
            (
                r"(?:implement|create|build)\s+(\w+).*(?:don\'t|do not|avoid)\s+\1",
                "Contradictory implementation instructions",
            ),
            (
                r"(?:use|enable)\s+(\w+).*(?:disable|remove|delete)\s+\1",
                "Contradictory usage instructions",
            ),
        ]
                ...
                issues.append(
                    CoherenceIssue(
                        ...
                        description=message.format(
                            match.group(1) if "{}" in message else ""
                        ),
    ...
    def calculate_coherence_score(
        self, contradictions: list[str], impossibilities: list[str]
    ) -> float:
    ...
    def detect_impossibilities(self, request: str, context: dict) -> list[str]:
        ...
        if "workspace_files" in context:
            impossibilities.extend(
                self._check_file_conflicts(request, context["workspace_files"])
            )

        # Check dependency conflicts
        if "dependencies" in context:
            impossibilities.extend(
                self._check_dependency_conflicts(request, context["dependencies"])
            )
        ...
        if "create" in request_lower and "delete" in request_lower:
            if any(word in request_lower for word in ["file", "directory", "folder"]):
                contradictions.append(
                    "Contradiction: create and delete operations on same target"
                )
    ...
    def _check_file_conflicts(
        self, request: str, workspace_files: set[str]
    ) -> list[str]:
    ...
    def _check_dependency_conflicts(
        self, request: str, dependencies: dict
    ) -> list[str]:
        ...
        package_refs = re.findall(
            r"(?:install|add|require)\s+([^\s,]+)", request, re.IGNORECASE
        )

        for package in package_refs:
            if package in dependencies:
                conflicts.append(
                    f"Package {package} already installed with version {dependencies[package]}"
                )
    ...
    def _check_resource_availability(self, request: str) -> list[str]:
        ...
        if re.search(
            r"(?:process|analyze)\s+(?:millions?|billions?)", request, re.IGNORECASE
        ):
