...
class PrerequisiteChecker:
    ...
    def check_context_completeness(
        ...
        is_ready = (
            len(missing_info) == 0 and len(workspace_issues) == 0 and success_prob > 0.7
        )
    ...
    def estimate_success_probability(
        self, missing_info: list[str], workspace_issues: list[str]
    ) -> float:
        ...
        final_probability = max(
            0.0, min(1.0, base_probability - info_penalty - workspace_penalty)
        )
    ...
    def _check_missing_file_references(self, request: str, context: dict) -> list[str]:
        ...
        for file_ref in referenced_files:
            if file_ref and not any(
                file_ref in existing for existing in workspace_files
            ):
    ...
    def _validate_workspace_structure(self, request: str) -> list[str]:
        ...
        if any(op in request.lower() for op in ["refactor", "analyze code", "test"]):
            if not any(
                os.path.exists(d) for d in ["src/", "lib/", "*.py", "*.js", "*.ts"]
            ):
        ...
        if any(op in request.lower() for op in ["install", "dependency", "package"]):
            if not any(
                ...
                issues.append(
                    "Package operation requested but no package configuration found"
                )
    ...
    def _detect_ambiguous_operations(self, request: str) -> list[str]:
        ...
        if any(word in request.lower() for word in ["all", "some", "many", "few"]):
            ambiguous.append(
                "Vague quantifiers found - specify exact counts or criteria"
            )
        ...
        if (
            any(op in request.lower() for op in ["update", "modify", "change"])
            and "to" not in request.lower()
        ):
    ...
    def _is_dependency_available(
        self, package: str, dep_type: str, context: dict
    ) -> bool:
    ...
    def _find_om_executable(self) -> str | None:
        ...
        for pattern in file_patterns:
            ...
            for match in matches:
                if self._estimate_file_size(request, match) > 200:
                    violations.append(
                        f"Potential CLU violation: {match} may exceed 200 lines"
                    )
