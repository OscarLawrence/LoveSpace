...
class PatternMatcher:
    ...
    def find_patterns(
        self, text: str, pattern_types: list[str] | None = None
    ) -> list[PatternMatch]:
    ...
    def validate_pattern_compliance(
        self, text: str, required_patterns: list[str], forbidden_patterns: list[str]
    ) -> ValidationResult:
        ...
        for forbidden in forbidden_patterns:
            if forbidden in self.validation_patterns:
                ...
                if matches:
                    issues.append(
                        f"Forbidden pattern found: {forbidden} ({len(matches)} occurrences)"
                    )
            ...
            for match in matches:
                anti_pattern_matches.append(
                    PatternMatch(
                        ...
                        context_span=self._get_context_span(
    ...
    def _find_pattern_matches(
        self, text: str, pattern_id: str, pattern_info: dict
    ) -> list[PatternMatch]:
        ...
        for match in regex_matches:
            confidence = self._calculate_match_confidence(
                match, pattern_type, pattern_info
            )
    ...
    def _calculate_match_confidence(
        self, match, pattern_type: str, pattern_info: dict
    ) -> float:
    ...
    def suggest_pattern_improvements(self, matches: list[PatternMatch]) -> list[str]:
        ...
        for pattern_id, pattern_matches in pattern_groups.items():
            ...
            if pattern_id == "file_size_violation" and count > 0:
                suggestions.append(
                    f"Split file into smaller modules ({count} size violations)"
                )

            elif pattern_id == "contradiction_indicators" and count > 3:
                suggestions.append(
                    "Review absolute statements for potential contradictions"
                )
