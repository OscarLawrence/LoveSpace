...
class OMIntegration:
    ...
    def execute_om_with_validation(
        ...
        try:
            ...
            post_validation = self._validate_om_output(
    ...
    def get_om_workspace_status(self) -> dict[str, Any]:
        ...
        success, output, validation = self.execute_om_with_validation(
            "workspace", ["status"]
        )
    ...
    def validate_workspace_modules(self) -> ValidationResult:
        ...
        healthy_modules = sum(
            1 for module_status in modules.values() if module_status == "healthy"
        )
    ...
    def _check_command_prerequisites(
        self, command: str, args: list[str]
    ) -> ValidationResult:
        ...
        return ValidationResult(
            status=ValidationStatus.PASSED, message="Prerequisites validated", score=1.0
        )

    def _validate_om_output(
