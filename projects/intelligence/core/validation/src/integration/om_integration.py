...
class OMIntegration:
    ...
    def validate_command_request(
        self, command: str, args: list[str], context: dict = None
    ) -> ValidationResult:
        """Validate command request before execution."""
        request_text = f"{command} {' '.join(args)}"

        # Logical coherence validation
        coherence_issues = self.coherence_validator.validate_request(
            request_text, context
        )

        # Prerequisite checking
        prerequisite_status = self.prerequisite_checker.check_prerequisites(
            request_text, context
        )
    ...
    def post_command_hook(
        self, command: str, result: Any, context: dict = None
    ) -> dict:
    ...
    def continuous_monitoring_hook(self, context: dict) -> dict | None:
    ...
    def _calculate_overall_score(
        self, coherence_issues, prerequisite_status, token_analysis, halt_events
    ) -> float:
        """Calculate overall validation score."""
        base_score = 1.0
        ...
        unsatisfied_prereqs = sum(
            1 for status in prerequisite_status if not status.satisfied
        )
        ...
        final_score = (
            base_score - coherence_penalty - prerequisite_penalty - halt_penalty
        )
    ...
    def _generate_recommendations(
        self, coherence_issues, prerequisite_status, token_analysis
    ) -> list[str]:
    ...
    def _verify_command_outcome(
        ...
        if command == "code":
            verification["metrics"]["files_processed"] = getattr(
                result, "files_processed", 0
            )
        elif command == "workspace":
            verification["metrics"]["modules_analyzed"] = getattr(
                result, "modules_analyzed", 0
            )
    ...
    def _check_system_health(self, context: dict) -> dict:
        ...
        health = {"status": "healthy", "warnings": [], "critical_issues": []}
        ...
        if token_usage > 90:
            health["critical_issues"].append(
                f"Token budget nearly exhausted: {token_usage}%"
            )
        elif token_usage > 75:
    ...
    def get_validation_metrics(self) -> dict:
        ...
        return {
            ...
            "token_usage": {
                ...
                "remaining": self.token_optimizer.daily_budget
                - self.token_optimizer.used_tokens,
            },
