...
class HaltController:
    ...
    def evaluate_halt_condition(
        ...
        should_halt = condition["auto_halt"] and confidence >= condition["threshold"]

        if should_halt:
            self._trigger_halt(
                session_id, reason, confidence, context, condition["recoverable"]
            )
    ...
    def check_contradiction_halt(
        self, session_id: str, contradictions: list[Any]
    ) -> bool:
        ...
        return self.evaluate_halt_condition(
            session_id, HaltReason.LOGICAL_CONTRADICTION, avg_confidence, context
        )

    def check_token_limit_halt(
        self, session_id: str, current_tokens: int, max_tokens: int
    ) -> bool:
        ...
        return self.evaluate_halt_condition(
            session_id, HaltReason.TOKEN_LIMIT, usage_ratio, context
        )

    def check_context_halt(
        self, session_id: str, completeness_score: float, missing_critical: list[str]
    ) -> bool:
        ...
        if missing_critical:
            return self.evaluate_halt_condition(
                session_id, HaltReason.MISSING_CONTEXT, halt_confidence, context
            )
    ...
    def _get_halt_message(self, reason: HaltReason, context: dict[str, Any]) -> str:
        ...
        messages = {
            HaltReason.LOGICAL_CONTRADICTION: f"Logical contradictions detected (confidence: {context.get('avg_confidence', 0):.2f})",
            HaltReason.MISSING_CONTEXT: f"Critical context missing: {', '.join(context.get('missing_critical', []))}",
            HaltReason.TOKEN_LIMIT: f"Token limit approached ({context.get('usage_ratio', 0):.1%})",
            HaltReason.CRITICAL_ERROR: f"Critical error encountered: {context.get('error', 'Unknown')}",
            HaltReason.USER_REQUEST: f"User requested halt: {context.get('reason', 'No reason provided')}",
        }
    ...
    def get_session_status(self, session_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "last_halt": session.halt_events[-1].__dict__
    ...
    def get_halt_statistics(self) -> dict[str, Any]:
        ...
        total_halts = sum(
            len(session.halt_events) for session in self.active_sessions.values()
        )
        ...
        return {
            ...
            "active_sessions": len(
                [s for s in self.active_sessions.values() if s.active]
            ),
        }

    def configure_halt_condition(
        self, reason: HaltReason, threshold: float, auto_halt: bool, recoverable: bool
    ) -> None:
