...
class CommandTrainer:
    ...
    def get_training_module(self, module_name: str) -> TrainingModule | None:
        ...
        for module in self.training_modules.values():
            for example in module.commands:
                if (
                    ...
                    and len(example.args) <= len(args)
                    and all(arg in args for arg in example.args[: len(example.args)])
                ):
                    return example
    ...
    def _validate_command_structure(
        self, command: str, args: list[str], example: CommandExample
    ) -> tuple[bool, str]:
    ...
    def assess_command_knowledge(
        ...
        for difficulty in ["basic", "intermediate", "advanced"]:
            if total_by_difficulty[difficulty] > 0:
                score = (
                    completed_by_difficulty[difficulty]
