...
class CodebaseNavigator:
    ...
    def get_module_recommendations(self, role: str = "new_developer") -> list[str]:
        ...
        if role == "new_developer":
            ...
            documented_modules = [
                m.name
                for m in self.navigation_map.modules
    ...
    def get_exploration_guide(self, module_name: str) -> dict:
        ...
        module = next(
            (m for m in self.navigation_map.modules if m.name == module_name), None
        )
        if not module:
    ...
    def _analyze_module(self, file_path: str) -> ModuleInfo | None:
    ...
    def _identify_entry_points(self, modules: list[ModuleInfo]) -> list[str]:
        ...
        cli_modules = [
            m.name
            for m in modules
            if any(keyword in m.name.lower() for keyword in ["cli", "main", "app"])
        ]
        entry_points.extend(cli_modules)

        # Look for __init__.py files with substantial content
        init_modules = [
            m.name for m in modules if m.name.endswith("__init__") and m.size_loc > 10
        ]
    ...
    def _generate_architecture_overview(
        self, modules: list[ModuleInfo], dependency_graph: dict[str, list[str]]
    ) -> str:
        ...
        overview.append(
            f"Codebase Overview: {total_modules} modules, {total_loc} lines of code"
        )
