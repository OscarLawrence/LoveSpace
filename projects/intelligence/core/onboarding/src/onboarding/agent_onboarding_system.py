...
class AgentOnboardingSystem:
    ...
    def get_next_training_module(self, agent_id: str) -> dict | None:
    ...
    def complete_training_module(self, agent_id: str, module_results: dict) -> bool:
        ...
        if not self._validate_module_completion(progress.current_stage, module_results):
            progress.issues_encountered.append(
                f"Failed validation for {progress.current_stage}"
            )
            return False
        ...
        progress.progress_percent = (
            len(progress.completed_stages) / len(self.onboarding_stages)
        ) * 100

        self._save_progress(progress)
        return True

    def get_onboarding_status(self, agent_id: str) -> OnboardingProgress | None:
    ...
    def _save_progress(self, progress: OnboardingProgress):
        ...
        try:
            progress_file = os.path.join(
                self.workspace_root, f"onboarding_progress_{progress.agent_id}.json"
            )
    ...
    def load_progress(self, agent_id: str) -> OnboardingProgress | None:
        """Load progress from persistent storage."""
        try:
            progress_file = os.path.join(
                self.workspace_root, f"onboarding_progress_{agent_id}.json"
            )
