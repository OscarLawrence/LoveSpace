...
class GradientDescentOptimizer:
    ...
    def _calculate_performance_gradient(
        ...
        for metric in context.current_metrics:
            if metric in recent[0] and metric in recent[1]:
                gradient[metric] = (recent[1][metric] - recent[0][metric]) / max(
                    abs(recent[0][metric]), 1e-6
                )
    ...
    def _estimate_improvement(
        ...
        current_performance = sum(context.current_metrics.values()) / len(
            context.current_metrics
        )
        historical_avg = sum(
            sum(perf.values()) / len(perf)
            for perf in context.historical_performance[-5:]
        ) / min(len(context.historical_performance), 5)

        base_improvement = (current_performance - historical_avg) / max(
            abs(historical_avg), 1e-6
        )
