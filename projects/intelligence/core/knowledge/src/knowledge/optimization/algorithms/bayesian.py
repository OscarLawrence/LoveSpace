...
class BayesianOptimizer:
    ...
    async def optimize(
        ...
        for strategy in candidate_strategies:
            ...
            predicted_performance = self._predict_performance_bayesian(
                parameters, context
            )
            acquisition_score = self._calculate_acquisition_score(
                predicted_performance, context
            )
    ...
    def _predict_performance_bayesian(
        self, parameters: dict[str, Any], context: OptimizationContext
    ) -> float:
    ...
    def _calculate_acquisition_score(
        self, predicted_performance: float, context: OptimizationContext
    ) -> float:
        """Calculate acquisition function score"""
        current_best = (
            max(context.current_metrics.values()) if context.current_metrics else 0.5
        )
