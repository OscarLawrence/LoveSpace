...
class GeneticOptimizer:
    ...
    async def optimize(
        self, context: OptimizationContext, analysis: dict[str, Any]
    ) -> OptimizationDecision:
        """Genetic algorithm optimization"""
        population = [
            self._generate_random_parameters() for _ in range(self.population_size)
        ]

        for generation in range(self.generations):
            fitness_scores = [
                self._evaluate_fitness(individual, context) for individual in population
            ]
            population = self._genetic_selection_reproduction(
    ...
    def _evaluate_fitness(
        self, individual: dict[str, Any], context: OptimizationContext
    ) -> float:
        ...
        historical_avg = sum(
            ...
            for perf in context.historical_performance[-3:]
        ...
        for param, value in individual.items():
            if (
                param == "batch_size"
                and 32 <= value <= 64
                or param == "learning_rate"
                and 0.001 <= value <= 0.01
            ):
                parameter_score += 0.1
    ...
    def _genetic_selection_reproduction(
        ...
        for key, value in mutated.items():
            if random.random() < 0.3:  # 30% chance to mutate each parameter
                if key == "batch_size":
                    ...
                    mutated[key] = max(
                        0.0001, min(0.1, value * random.uniform(0.5, 2.0))
                    )
                elif key == "threads":
                    mutated[key] = max(1, min(8, value + random.randint(-2, 2)))
                elif key == "cache_size":
                    mutated[key] = max(
                        256, min(2048, value + random.randint(-256, 256))
                    )
