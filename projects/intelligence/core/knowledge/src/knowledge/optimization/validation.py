...
class DecisionValidator:
    ...
    async def validate_decision(
        self, decision: OptimizationDecision, context: OptimizationContext
        ...
        if decision.confidence < self.validation_rules["confidence_threshold"]:
            validated_decision.confidence = self.validation_rules[
                "confidence_threshold"
            ]
            validated_decision.reasoning += (
                " (confidence adjusted to minimum threshold)"
            )

        # Check improvement threshold
        if (
            decision.expected_improvement
            < self.validation_rules["improvement_threshold"]
        ):
            validated_decision.expected_improvement = self.validation_rules[
                "improvement_threshold"
            ]
            validated_decision.reasoning += (
                " (improvement adjusted to minimum threshold)"
            )
        ...
        constraint_violations = self._check_constraint_compliance(
            validated_decision, context
        )
        if constraint_violations:
            validated_decision.reasoning += (
                f" (constraints adjusted: {', '.join(constraint_violations)})"
            )
