...
class RealTimeOptimizationEngine:
    ...
    async def optimize(self, context: OptimizationContext) -> OptimizationDecision:
        ...
        algorithm_name = self.algorithm_selector.select_algorithm(
        ...
        # Validate decision
        validated_decision = await self.decision_validator.validate_decision(
            decision, context
        )
    ...
    async def _update_learning_state(
        self, context: OptimizationContext, decision: OptimizationDecision
    ):
    ...
    def _encode_context(self, context: OptimizationContext) -> str:
        ...
        performance_level = (
            "high" if sum(context.current_metrics.values()) > 2.0 else "low"
        )
    ...
    def get_optimization_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_decisions": list(self.strategy_history)[-5:]
