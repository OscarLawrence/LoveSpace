...
class MemoryConsolidation:
    ...
    def consolidate_memory(
        ...
        successful_patterns = [
            pattern_id
            for pattern_id, pattern in performance_patterns.items()
        ...
        for i, pattern1 in enumerate(successful_patterns):
            for pattern2 in successful_patterns[i + 1 :]:
                ...
                if correlation_strength > 0.5:
                    ...
                    self.pattern_correlations[pattern1].append(
                        (pattern2, correlation_strength)
                    )
    ...
    def _calculate_pattern_correlation(
        ...
        context_similarity = (
            1.0 if pattern1.context_type == pattern2.context_type else 0.0
        )
