...
class BatchEmbeddingManager:
    ...
    def batch_context_injection(
        ...
        if dedupe:
            ...
            original_count = deduped_count = (
                len(all_functions) + len(all_patterns) + len(all_docs)
            )
    ...
    def format_batch_output(
        self, result: BatchResult, selected_format: FormatType, queries: list[str]
    ) -> str:
        ...
        if result.functions:
            ...
            format_prefix = (
                "funcs" if selected_format != FormatType.NATURAL else "functions"
            )
            context_parts.append(
                f"{format_prefix}:{len(result.functions)}:{':'.join(func_refs)}"
            )
        ...
        if result.patterns:
            ...
            for pattern in result.patterns:
                ...
                if pattern.usage_context:
                    usage = (
                        f":{pattern.usage_context[:30]}"
                        if len(pattern.usage_context) > 30
                        else f":{pattern.usage_context}"
                    )

                pattern_refs.append(
                    f"{pattern.name}:{pattern.pattern_type}:success_{pattern.success_count}{usage}"
                )
            context_parts.append(
                f"patterns:{len(result.patterns)}:{':'.join(pattern_refs)}"
            )
...
def find_similar_functions_with_embedding(
...
def find_similar_patterns_with_embedding(
    self, embedding: np.ndarray, limit: int = 5
) -> list[Pattern]:
    """Find similar patterns using pre-calculated embedding."""
    rows = self.db.conn.execute("""
...
EmbeddingManager.find_similar_functions_with_embedding = (
    find_similar_functions_with_embedding
)
EmbeddingManager.find_similar_patterns_with_embedding = (
    find_similar_patterns_with_embedding
)
