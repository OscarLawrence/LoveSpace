    ...
    def validate_reasoning_chain(self, reasoning_steps: list[str]) -> CoherenceResult:
        ...
        for i, step in enumerate(reasoning_steps):
            ...
            for j, prev_step in enumerate(reasoning_steps[:i]):
                cross_contradictions = self._detect_cross_contradictions(
                    prev_step, step
                )
                contradictions.extend(
                    [f"Steps {j + 1}-{i + 1}: {c}" for c in cross_contradictions]
                )
        ...
        for pos, neg in opposites:
            if pos in words and neg in words:
                contradictions.append(
                    f"Direct contradiction: '{pos}' and '{neg}' in same statement"
                )

        return contradictions

    def _detect_cross_contradictions(
        ...
        if "not" in s1_words and any(
            word in s2_words for word in s1_words if word != "not"
        ):
        ...
        if ("all" in s1_lower or "every" in s1_lower) and (
            "contain" in s2_lower or "have" in s2_lower
        ):
            if any(word in s1_lower for word in ["perfect", "logical", "consistent"]):
                if any(
                    word in s2_lower
                    for word in ["contradiction", "error", "inconsistent"]
                ):
                    contradictions.append(
                        "Universal claim contradicted by existence claim"
                    )
        ...
        if "impossible" in s1_lower or "cannot" in s1_lower:
            if "therefore" in s2_lower or "thus" in s2_lower:
                contradictions.append(
                    "Impossible premise leads to definitive conclusion"
                )

        return contradictions

    def _calculate_coherence_score(
        self, reasoning_steps: list[str], contradictions: list[str]
    ) -> float:
        ...
        if contradictions:
            ...
            cross_contradictions = [
                c for c in contradictions if "between" in c or "Steps" in c
            ]
        ...
        avg_length = sum(len(step.split()) for step in reasoning_steps) / len(
            reasoning_steps
        )
        if avg_length < 5:
            base_score -= 0.3
