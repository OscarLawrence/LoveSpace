...
from core.session_manager import CollaborationStage, SessionManager
...
@contract_enforced(
    preconditions=["session_id is valid UUID"], description="Get session by ID"
)
...
@contract_enforced(description="Get task tree for session")
async def get_tasks(
    session_id: str, session_manager: SessionManager = Depends(get_session_manager)
):
